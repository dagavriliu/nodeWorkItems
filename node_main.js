...
const settings = require('./config/JiraOptions.json');
const proxyTargetEndpoint = settings.endpoint + "/";
...
http.createServer(function(req, res) {
    if (req.url.startsWith('/proxy/')) {
        handleProxyRequests(req, res, '/proxy/', proxyTargetEndpoint);
...
function handleProxyRequests(req, resp, pathPrefix, targetEndpoint) {
    var url = targetEndpoint + req.url.substr(pathPrefix.length);
    console.log(`proxy ${req.method} ->  ${url}`);
    var piped = null;

    if (req.method == 'GET') {
        piped = request.get(url);
    } else if (req.method == 'POST') {
        piped = request.post(url);
    } else if (req.method == 'PUT') {
        piped = request.put(url);
    }
